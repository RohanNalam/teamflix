...
export default function SessionsPage() {
  ...
  const handleDelete = async (id: string, e: React.MouseEvent) => {
    e.stopPropagation()
    await supabase.from('sessions').delete().eq('id', id).eq('user_id', getDeviceId())
  ...
  return (
    <div className="px-8 py-8">
      ...
      <div className="relative mb-6 max-w-sm">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search sessions…" className="w-full pl-10 pr-4 py-2.5 rounded-xl text-sm border outline-none" style={{ background: 'var(--surface)', borderColor: 'var(--border)', color: 'var(--foreground)' }} />
      </div>
      ...
      <div className="rounded-2xl border overflow-hidden" style={{ borderColor: 'var(--border)', background: 'var(--surface)' }}>
        {filtered.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-xs" style={{ color: 'var(--muted)' }}>Create a session or type a prompt on the dashboard.</p>
          </div>
        ) : (
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border)' }}>
                {['Session', 'Status', 'Agent', 'Created', ''].map(h => (
                ...
                return (
                  <tr key={s.id} onClick={() => window.location.href = `/dashboard/sessions/${s.id}`} className="hover:bg-white/3 transition-colors cursor-pointer" style={i < filtered.length - 1 ? { borderBottom: '1px solid var(--border)' } : {}}>
                    ...
                    <td className="px-5 py-4 text-sm" style={{ color: 'var(--muted)' }}>
                      <span className="flex items-center gap-1.5"><Clock size={13} />{new Date(s.created_at).toLocaleDateString()}</span>
                    </td>
                    <td className="px-5 py-4">
                      <button onClick={e => handleDelete(s.id, e)} className="p-1.5 rounded-lg hover:bg-red-500/10 transition-colors group">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: '#00000080' }}>
          <div className="w-full max-w-md rounded-2xl border p-6" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setShowModal(false)} className="p-1.5 rounded-lg hover:bg-white/10"><X size={16} style={{ color: 'var(--muted)' }} /></button>
            </div>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold mb-1.5 uppercase tracking-wider" style={{ color: 'var(--muted)' }}>Session name</label>
                <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. fix-auth-bug" className="w-full px-3 py-2.5 rounded-xl border text-sm outline-none font-mono" style={{ background: 'var(--surface-2)', borderColor: 'var(--border)', color: 'var(--foreground)' }} />
              </div>
              <div>
                <label className="block text-xs font-semibold mb-1.5 uppercase tracking-wider" style={{ color: 'var(--muted)' }}>Agent</label>
                <select value={agent} onChange={e => setAgent(e.target.value)} className="w-full px-3 py-2.5 rounded-xl border text-sm outline-none" style={{ background: 'var(--surface-2)', borderColor: 'var(--border)', color: 'var(--foreground)' }}>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleCreate} disabled={!name.trim() || loading} className="flex-1 py-2.5 rounded-xl text-sm font-semibold transition-opacity hover:opacity-80 disabled:opacity-40" style={{ background: 'var(--accent)', color: '#1a1910' }}>
