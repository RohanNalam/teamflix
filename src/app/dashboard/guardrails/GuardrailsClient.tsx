...
import { toggleGuardrail } from '@/app/actions/guardrails'
...
const severityColor: Record<string, string> = {
  high: '#ef4444', medium: '#D9D7B6', low: '#878672'
}

export default function GuardrailsClient({ savedStates }: { savedStates: Record<string, boolean> }) {
  const [states, setStates] = useState<Record<string, boolean>>(() => {
    const init: Record<string, boolean> = {}
    RULES.forEach(r => { init[r.id] = r.id in savedStates ? savedStates[r.id] : r.defaultEnabled })
    return init
  })
  ...
  const toggle = (id: string) => {
    ...
    startTransition(async () => {
      await toggleGuardrail(id, next)
      const rule = RULES.find(r => r.id === id)
      toast(`${rule?.label} ${next ? 'enabled' : 'disabled'}`, next ? 'success' : 'error')
    })
  }
        ...
        {RULES.map(rule => (
          <div key={rule.id} className="rounded-2xl border p-5 flex items-center gap-4 transition-colors hover:border-[#878672]/20" style={{ background: 'var(--surface)', borderColor: 'var(--border)' }}>
            <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: severityColor[rule.severity] + '18' }}>
              {rule.severity === 'high'
                ? <AlertTriangle size={16} style={{ color: severityColor[rule.severity] }} />
                : <Shield size={16} style={{ color: severityColor[rule.severity] }} />
              }
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5">
                <span className="text-sm font-semibold" style={{ color: 'var(--foreground)' }}>{rule.label}</span>
                <span className="text-[10px] px-1.5 py-0.5 rounded font-bold uppercase tracking-wide" style={{ color: severityColor[rule.severity], background: severityColor[rule.severity] + '18' }}>
                  {rule.severity}
                </span>
              </div>
            ...
            <button
              ...
              style={{ width: 40, height: 22, background: states[rule.id] ? 'var(--accent)' : 'var(--border)' }}
            >
              <span
                className="absolute top-1 rounded-full transition-all"
                style={{ width: 14, height: 14, background: '#1a1910', left: states[rule.id] ? 22 : 4 }}
              />
            </button>
          </div>
        ))}
